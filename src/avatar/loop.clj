(ns avatar.loop
  ...
  through avatar.cosci -> avatar.governor -> commit+publish. `run!` reduces
  ...
  queue — see avatar.murakumo's docstring. A SECOND, avatar-specific limit:
  when AVATAR_PARTS_CIDS is unset/blank, avatar.generate/round-candidates
  returns 0 candidates, so submit-round! simply submits nothing that tick
  ...
  (:require [clojure.java.io :as io]
            ...
            [avatar.judge :as judge]
            [avatar.cosci :as cosci]
            [avatar.governor :as governor]
            [avatar.datalad :as datalad]
            [avatar.ledger :as ledger]
            [avatar.publish :as publish]
            [avatar.aozora :as aozora]))

(def lease-path ".avatar/lease")
(def state-path ".avatar/state.edn")
...
(defn submit-round!
  ...
  logs to the ledger and continues with however many did submit). If
  avatar.generate/round-candidates returns [] (AVATAR_PARTS_CIDS unset —
  nothing to compose), this simply submits nothing and advances no state
  beyond clearing :pending, same as any other empty round."
  [state]
...
(defn- fetch-result
  "One pending entry -> a avatar.cosci/reflect-shaped result map, or nil if
...
(defn- asset-title [gene]
  (str/join ", " (remove nil? [(:expression gene) (:material-variant gene)])))
...
(defn settle-round!
  ...
  avatar.publisher/mock-publisher in tests."
   ...
   (let [p (persona/load-persona)
     ...
     (when winner
       (let [{:keys [result score]} winner
             ...
             asset {:id (str "avatar-" (str/replace job-id #"[^a-zA-Z0-9-]" "-"))
...
(defn tick!
  ...
  sent — may be zero candidates if AVATAR_PARTS_CIDS is unset, see
  avatar.generate) / :waiting (pending round, not yet settled) / :settled
  (round judged+committed-or-held) / :idle (nothing to do, budget
  exhausted)."
  ([] (tick! nil))
  ([publisher]
