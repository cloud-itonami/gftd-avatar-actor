(ns avatar.cosci
  "Google co-scientist / AlphaEvolve-shaped tournament over one round of VRM
  compose candidates (ADR-2607122200 §3), ported-in-shape from
  ...
  slightly-worse asset, it's a disqualified one.
    ...
    Generation  -> avatar.generate/round-candidates (closed gene pool over
                   expression/material-variant/accessory compose config,
                   0 candidates if AVATAR_PARTS_CIDS is unset)
    Reflection  -> hard gates: job actually :done, artifact fetched,
                   declared format matches, no upstream safety flag
    Ranking     -> Elo on avatar.judge scores (pairwise, draw-tolerant)
...
(defn rank
  ...
  missing entries when avatar.judge/score! failed) -> ranked vector,
...
(defn evolve-round
  "ranked -> {:next-round n+1 :elite-gene {...}} for avatar.generate's next
...
(defn run-round
  ...
  :ranked (all candidates failed reflection, OR the round had zero
  candidates because AVATAR_PARTS_CIDS was unset — see avatar.generate) is a
  valid, expected outcome — the loop holds the whole round rather than
  committing a disqualified candidate."
